	...
	public static void setMimicConfig(Level level) throws IOException {
		...
		if (!dir.isDirectory() || !flowerMimicsFile.isFile() || !flowerItemDropsFile.isFile()) {
			...
			for (String line : flowerMimicsContent.split("\n")) {
				...
				ResourceLocation flowerBlockResourceLocation = ResourceLocation.parse(flowerBlockRlString);

				Optional<Holder.Reference<Block>> flowerBlockOptionalReference = level.registryAccess().lookupOrThrow(BuiltInRegistries.BLOCK.key()).get(flowerBlockResourceLocation);
				if (flowerBlockOptionalReference.isEmpty()) {
					Variables.logger.warn("[" + Reference.NAME + "] 1. Unable to find flower '" + flowerBlockRlString + "' in registry.");
					continue;
				}
				...
				ResourceLocation mimicEntityTypeResourceLocation = ResourceLocation.parse(mimicEntityTypeString);

				Optional<Holder.Reference<EntityType<?>>> mimicEntityTypeOptionalReference = level.registryAccess().lookupOrThrow(BuiltInRegistries.ENTITY_TYPE.key()).get(mimicEntityTypeResourceLocation);
			...
			for (String line : flowerItemDropsContent.split("\n")) {
				...
				try {
					...
					optionalItemStack.ifPresent(itemStack -> flowerMimicDrops.put(flowerBlockOptionalReference.get().value(), itemStack.copy()));
				}
